...
{
    public partial class ReporteTramitesSuspendidos : Utilerias.Comun
    {
        ...
        protected void btnFiltroMes_Click(object sender, EventArgs e)
        {
            ...
            {
                int IdPromotoria = -1; //Convert.ToInt32(cbFlujos.SelectedItem.Value.ToString());
                int IdFlujo = Convert.ToInt32(cbFlujos.SelectedItem.Value.ToString());
                List<prosu.Tramites_Supendidos> tramites = rs.Tramites_Suspendidos(CalDesde.Date, CalHasta.Date, IdFlujo, IdPromotoria);
